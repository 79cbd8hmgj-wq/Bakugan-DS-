...
from bakugan_ds.analysis.model import Component, PointerReference, StringRecord

_ASCII_PATTERN = re.compile(rb"[ -~]{4,}\x00")


def extract_ascii_strings(
    component: Component, *, minimum_length: int = 4
) -> tuple[StringRecord, ...]:
    if minimum_length < 1:
...
def filter_strings(
    records: tuple[StringRecord, ...], keywords: tuple[str, ...]
) -> tuple[StringRecord, ...]:
...
def find_pointer_references(
    components: tuple[Component, ...], target_address: int
