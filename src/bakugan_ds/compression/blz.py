...
def decompress_blz_in_place(data: Buffer) -> bytes:
    ...
    Unlike :func:`decompress_blz`, this routine reads and writes one shared
    buffer. A stream may therefore decode differently after overwriting bytes
    that the decoder has not consumed yet.
    """
...
def _encode_blz_suffix(data: bytes) -> bytes:
    """Encode one suffix into the reverse LZ stream consumed by BLZ."""
    ...
    while cursor < len(reversed_data):
        ...
        # One-token lazy matching avoids a short current reference when a
        # longer reference begins after one literal. This is required not
        # only for size but for the DS loader's in-place overwrite boundary.
