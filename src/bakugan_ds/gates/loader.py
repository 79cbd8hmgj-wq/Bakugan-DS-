...
class CacheLayout:
    ...
    def validate(self) -> None:
        if (
            ...
            + self.preserved_bss_size
            != self.module_start
        ):
            raise WorkspaceError(
                "System 2.0 module start does not follow preserved BSS"
            )
        ...
        if self.cache_start + self.cache_size != self.arena_low:
            raise WorkspaceError(
                "battle arena low boundary does not follow the cache"
            )
...
class NitroFsOperation:
    ...
    def validate(self) -> None:
        ...
        for label, value in (
            ...
            if not value.strip():
                raise WorkspaceError(
                    f"NitroFS operation {label} must be nonempty"
                )
...
def append_validated_trailer(
    ...
    *,
    ...
    if len(original_raw) >= TRAILER_SIZE and original_raw[-TRAILER_SIZE:].startswith(
        G2DT_MAGIC
    ):
...
def validate_overlay_expansion(
    ...
    *,
    ...
    if len(original) != layout.original_payload_size:
        raise WorkspaceError(
            "original overlay payload size does not match layout"
        )
    ...
    expected_expanded_size = (
        layout.original_payload_size
        + layout.preserved_bss_size
        + layout.module_size
    )
    ...
    if expanded[: len(original)] != original:
        raise WorkspaceError(
            "expanded overlay does not preserve original payload"
        )
    ...
    if layout.overlay_base + len(expanded) != layout.cache_start:
        raise WorkspaceError(
            "expanded overlay does not end at the cache start"
        )
