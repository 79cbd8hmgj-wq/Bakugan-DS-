...
SYSTEM2_STORAGE_DECISION = StorageDecision(
    ...
    candidates=(
        StorageCandidate(
            ...
            confirmed_requirements=(
                "Use a 4,152-byte G2DT trailer appended after the existing raw LZ10 stream of font/mes_CardName.mes.",
                "Native LZ10 decoding remains byte-identical because decoding stops at the 6,524-byte declared output size.",
                "Read one 32-byte header and one 40-byte selected record into a 72-byte stack buffer during battle construction.",
                "Reject missing magic, unsupported version, bad record geometry, invalid card ID, or checksum failure and use legacy behavior.",
                "Rebuild by appending the exact trailer to the original raw file payload before deterministic FAT repacking.",
            ),
            unresolved_requirements=(),
            risks=(
                "Repeated raw NitroFS reads are slower than a battle-local cache.",
                "The fallback is suitable for the first prototype but should not perform file I/O in frame-critical loops.",
            ),
            viable=True,
        ),
        StorageCandidate(
            ...
            scope="not_selected",
            confirmed_requirements=(
                "The decoded ARM9 contains 72 zero Gate-table bytes for global IDs 201 through 212.",
                "Overlay 7 is fixed at 467,360 decoded bytes and the current rebuilder rejects overlay growth.",
            ),
            unresolved_requirements=(
                "No safe full-roster code/data region is confirmed.",
                "The ARM9 runtime image is BLZ-compressed and the repository has no deterministic BLZ recompressor.",
        ...
        StorageCandidate(
            ...
            confirmed_requirements=(
                ...
                "Preserve the original 0x640-byte overlay-7 BSS as zero-backed writable bytes inside the expanded payload.",
                "Append a guarded 0x8000-byte System 2.0 code region at 0x0228BC20 through 0x02293C20.",
                "Set overlay-7 ram_size to 0x7A7E0 and reserve a new 0x40-byte BSS cache at 0x02293C20 through 0x02293C60.",
                "Move the ARM9 battle-arena low boundary from 0x0228BC20 to 0x02293C60 atomically with the overlay metadata changes.",
                "The arena high boundary is 0x023E0000, leaving 1,360,800 bytes after code and cache reservation.",
                "Zero the cache on overlay load and reset its valid flag when the battle object completes.",
    ...
    evidence=(
        "font/mes_CardName.mes is file ID 2762, raw size 2,840, decoded size 6,524, with 213 indexed names.",
        "The raw LZ10 stream reaches its declared output after 2,838 bytes and already has two ignored trailing zero bytes.",
        "Global IDs 1 through 103 are Gate Cards, so 103 fixed 40-byte records plus a 32-byte header require 4,152 bytes.",
        "ARM9 function 0x020061D8 returns 0x0228BC20 as the overlay-7 arena low boundary and 0x023E0000 as the main arena high boundary.",
        "Overlay-7 ram_size is 0x721A0 and BSS is 0x640; the hybrid layout changes them to 0x7A7E0 and 0x40 while preserving original BSS addresses.",
    ),
)
