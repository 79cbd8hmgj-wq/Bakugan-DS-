...
from bakugan_ds.util import require_range
            ...
            "files": [
                {
                    ...
                    "path": (
                        path_by_id[entry.file_id].path if entry.file_id in path_by_id else None
                    ),
...
def inspect_rom(path: Path, profile: RomProfile, require_supported: bool) -> RomInspection:
    identity = validate_rom(path, profile) if require_supported else read_rom_identity(path)
    supported = True if require_supported else _identity_matches_profile(identity, profile)
