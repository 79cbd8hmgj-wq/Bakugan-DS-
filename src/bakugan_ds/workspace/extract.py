...
from bakugan_ds.compression.lz10 import decompress_lz10, is_lz10
from bakugan_ds.errors import RomFormatError, WorkspaceError
...
from bakugan_ds.workspace.paths import ensure_unique_relative_paths
...
def _populate_workspace(
    ...
    inspection = inspect_rom(rom_path, profile, require_supported=True)
...
def extract_workspace(
    ...
    try:
        manifest = _populate_workspace(rom_path, profile, WorkspaceLayout.from_root(staging))
        if target.exists():
