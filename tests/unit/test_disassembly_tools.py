...
def test_find_module_params_parses_aligned_nitro_block() -> None:
    data = _module_params_bytes()
    params = find_module_params(data, base_address=0x02000000)
...
def test_overlay_layout_report_marks_shared_static_boundary() -> None:
    ...
    assert report["static_end"] == 0x02219440
    ...
    assert report["load_relations"] == [
        {"overlay_id": 2, "after_overlay_id": 0},
    ]
...
def test_render_labelled_bytes_preserves_prefix_and_splits_at_labels() -> None:
    ...
    assert "0x10, 0x11, 0x12, 0x13" in rendered
