...
def write_patch(tmp_path: Path, patches: list[dict[str, object]], profile: str = "b6re_rev0") -> Path:
    path = tmp_path / "patch.json"
    path.write_text(
        json.dumps({"format_version": 1, "profile_id": profile, "patches": patches}),
        encoding="utf-8",
    )
    return path
...
def test_load_patch_set_parses_binary_replacement(tmp_path: Path) -> None:
    ...
    assert patch_set.profile_id == "b6re_rev0"
...
def test_load_rejects_duplicate_patch_ids(tmp_path: Path) -> None:
    path = write_patch(
        tmp_path,
        [binary_patch("same", "arm9", 0, "00", "11"), binary_patch("same", "arm7", 0, "AA", "BB")],
    )
...
def test_apply_guarded_patches_to_all_target_types(tmp_path: Path) -> None:
    ...
    assert (workspace / "modified/overlays/overlay_007.bin").read_bytes() == bytes.fromhex("1020ABCD50")
...
def test_apply_rejects_profile_mismatch(tmp_path: Path) -> None:
    workspace = make_workspace(tmp_path)
    path = write_patch(tmp_path, [binary_patch("p1", "arm9", 0, "00", "11")], profile="other")
